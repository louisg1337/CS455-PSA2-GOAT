...
public class StudentNetworkSimulator extends NetworkSimulator {
    ...
    protected void aOutput(Message message) {
        Packet newPacket = new Packet(seqA, 0, 0, "");

        if (retransmit) {
            int check = checkSum(seqA, 0, previous);
            newPacket.setChecksum(check);
            newPacket.setPayload(previous);
            retransmit = false;
        } else {
            previous = message.getData();
            int check = checkSum(seqA, 0, message.getData());
            newPacket.setChecksum(check);
            newPacket.setPayload(message.getData());
        }
    ...
    protected void aInput(Packet packet) {
        // Got packet, stop the timer
        stopTimer(0);
        System.out.println("");
        System.out.println("///////////////////////////////");
        System.out.println("A INPUT: Received this packet back...");
        System.out.println(packet.toString());

        // Check the checksum
        int check = checkSum(packet.getSeqnum(), packet.getAcknum(), packet.getPayload());
        if (check != packet.getChecksum()) {
            // Checksum has error, retransmit current packet
            retransmit = true;
            return;
        }

        // Check to see if packet was acknowledged
        if (packet.getAcknum() == seqA + 1) {
            seqA += 1;
        } else {
            retransmit = true;
        }
    ...
    protected void aTimerInterrupt() {
        ...
        // If we get here, resend previous packet
        aOutput(new Message(previous));
    }
    ...
    protected void bInput(Packet packet) {
        ...
        int check = checkSum(packet.getSeqnum(), packet.getAcknum(), packet.getPayload());
        if (check != packet.getChecksum()) {
            System.out.println("Failed B checksum");
            // Corrupted... send ack for same packet
            toLayer3(1, new Packet(0, ackB, ackB));
            return;
        }

        // Check to see if ack works
        if (packet.getSeqnum() == ackB) {
            // Good ack, send to layer 5
            ...
            System.out.println("Failed B ack");
            // Bad ack, send current ack
            toLayer3(1, new Packet(0, ackB, ackB));
        }
